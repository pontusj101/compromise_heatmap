...
def create_random_instance(size='medium', horizon=150, rddl_path='content/'):
...
non-fluents simple_network {
    ...
    objects{
            host: '''
    instance_string += "{" + ", ".join([f"h{i}" for i in range(1, n_hosts + 1)]) + "}"
    instance_string += ''';
            credentials: '''
    instance_string += "{" + ", ".join([f"h{i}" for i in range(1, n_credentials + 1)]) + "}"
    instance_string += ''';
        };
...
def create_instance(instance_type='static', size='medium', horizon=150, rddl_path='content/'):
    if instance_type == 'static':
        create_static_instance(size=size, horizon=horizon, rddl_path=rddl_path)
    elif instance_type == 'random':
        create_random_instance(size=size, horizon=horizon, rddl_path=rddl_path)
