...
def simulation_worker(sim_id, log_window, max_start_time_step, rddl_path, random_cyber_agent_seed, debug_print):
    ...
    for step in range(myEnv.horizon):
        ...
        if step >= log_window:
            ...
            snapshot = Data(x=combined_features, edge_index=edge_index, y=labels)
...
def produce_training_data_parallel(use_saved_data=False, n_simulations=10, log_window=25, max_start_time_step=100, rddl_path='content/', random_cyber_agent_seed=42, debug_print=False):
    ...
    if use_saved_data:
        ...
        simulation_args = [(i, log_window, max_start_time_step, rddl_path, random_cyber_agent_seed, debug_print) for i in range(n_simulations)]
