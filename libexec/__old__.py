...
parser = argparse.ArgumentParser(
    description="Run different modes of the security simulation program."
)
...
parser.add_argument(
    "--bucket_name",
    type=str,
    default="heatmap-gnn-rddl",
    help="Name of the GCP bucket to use for storage.",
)

# Instance creation and training
parser.add_argument(
    "--min_size", type=int, default=4, help="Minimum number of hosts in each instance"
)
parser.add_argument(
    "--max_size",
    type=int,
    default=1500,
    help="Maximum number of hosts in each instance",
)
parser.add_argument(
    "--min_game_time",
    type=int,
    default=8,
    help="Min time horizon for the simulation and training.",
)  # small: 70, large: 500
parser.add_argument(
    "--max_game_time",
    type=int,
    default=1024,
    help="Max time horizon for the simulation and training. Will stop simulation early if whole graph is compromised.",
)  # small: 70, large: 500

# Instance creation
parser.add_argument(
    "--n_instances", type=int, default=256, help="Number of instances to create"
)
parser.add_argument(
    "--random_initial_compromise",
    action="store_true",
    help="Randomly choose all initial compromised hosts.",
)
parser.add_argument(
    "--n_init_compromised",
    type=int,
    default=3,
    help="Number of hosts initially compromised in each instance",
)
parser.add_argument(
    "--extra_host_host_connection_ratio",
    type=float,
    default=0.25,
    help="0.25 means that 25% of hosts will have more than one connection to another host.",
)

# Simulation
parser.add_argument(
    "-l", "--sim_log_window", type=int, default=64, help="Size of the logging window"
)
parser.add_argument(
    "--agent_type",
    default="random",
    ...
    help="Type of agent to use for simulation",
)
parser.add_argument(
    "--novelty_priority",
    type=int,
    default=2,
    help="Priority of newly discovered actions for less_random agent",
)
parser.add_argument(
    "--random_agent_seed", default=None, help="Seed for random cyber agent"
)
...
parser.add_argument(
    ...
    type=int,
    default=10,
    help="Penalty for wrong classification of compromised seqs",
)
parser.add_argument("--enable_wandb", action="store_true")
parser.add_argument(
    "--gnn_type",
    default="GAT",
    choices=["GAT", "RGCN", "GIN", "GCN", "GAT_LSTM"],
    help="Type of GNN to use for training",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=64,
    help="Number of uncompromised sequences to use",
)
parser.add_argument(
    "--train_log_window", type=int, default=64, help="Size of the logging window"
)
parser.add_argument(
    "--epochs", type=int, default=32, help="Number of epochs for GNN training"
)
parser.add_argument(
    "--learning_rate", type=float, default=0.0002, help="Learning rate for GNN training"
)
parser.add_argument(
    "--batch_size", type=int, default=128, help="Batch size for GNN training"
)
parser.add_argument(
    "--n_hidden_layer_1",
    type=int,
    default=128,
    help="Number of neurons in hidden layer 1 for GNN",
)
parser.add_argument(
    "--n_hidden_layer_2",
    type=int,
    default=128,
    help="Number of neurons in hidden layer 2 for GNN",
)
parser.add_argument(
    "--n_hidden_layer_3",
    type=int,
    default=128,
    help="Number of neurons in hidden layer 3 for GNN",
)
parser.add_argument(
    "--n_hidden_layer_4",
    type=int,
    default=0,
    help="Number of neurons in hidden layer 4 for GNN",
)
parser.add_argument(
    "--edge_embedding_dim",
    type=int,
    default=128,
    help="Edge embedding dimension for GAT",
)
parser.add_argument(
    "--heads_per_layer",
    type=int,
    default=2,
    help="Number of attention heads per layer for GAT",
)
parser.add_argument(
    "--lstm_hidden_dim",
    type=int,
    default=128,
    help="Number of neurons in LSTM hidden layer for GNN_LSTM",
)
parser.add_argument(
    "--checkpoint_file",
    type=str,
    default=None,
    help="Name of the checkpoint file to resume training from.",
)
...
parser.add_argument(
    "--model_filepath",
    type=str,
    default="models/model/GAT/log_window__hl_380,209,439_lstm_284_nsnpsht_128_lr_0.0002_bs_128_20231231_143651.pt",
    help="Path the model filename, relative to the bucket root.",
)
...
parser.add_argument(
    ...
    type=float,
    default=0.5,
    help="The threashold probability at which a predicted label is considered positive.",
)
parser.add_argument(
    "--predictor_type",
    default="gnn",
    choices=["gnn", "tabular", "none"],
    help="Type of predictor",
)
parser.add_argument(
    "--n_evaluation_sequences",
    type=int,
    default=32,
    help="Number of evaluation sequences to create",
)
...
parser.add_argument(
    "--animation_sequence_filepath",
    type=str,
    default="animation_sequences/log_window_64/256_nodes/960_snapshots/novelty/20231231_210248_3828.pkl",
    help="Path the animation sequence filename, relative to the bucket root.",
)
parser.add_argument(
    "--frames_per_second",
    type=int,
...
parser.add_argument(
    "--n_init_compromised_animate",
    type=int,
    default=1,
    help="Number of hosts initially compromised in each instance",
)
parser.add_argument(
    "--hide_prediction", action="store_true", help="Hide prediction in the animation."
)
parser.add_argument(
    "--hide_state",
    action="store_true",
    help="Hide the attacker's state in the animation.",
)
# and --predictor_filename and --predictor_type
...
max_start_time_step = args.sim_log_window + int(
    (args.max_game_time - args.sim_log_window) / 2
)
...
if "instance" in args.modes:
    logger.info("Creating new instance specification.")
    ...
    logger.info(
        f"{len(instance_rddl_filepaths)} instance specifications and graph indicies written to file."
    )

if "simulate" in args.modes:
    logger.info("Producing training data.")
    simulator = Simulator()
    ...
    logger.info(
        f'Training data produced and written to {config["training_sequence_dirpath"]}.'
    )
...
if "eval_seq" in args.modes:
    logger.info(
        f"Producing {args.n_evaluation_sequences} evaluation snapshot sequences."
    )
    instance_rddl_filepaths, graph_index_filepaths = create_instance(
        rddl_path=config["rddl_dirpath"],
    ...
    logger.info(
        f'Evaulation data produced and written to {config["evaluation_sequence_dirpath"]}.'
    )
...
if "anim_seq" in args.modes:
    logger.info("Producing single animantion snapshot sequence.")
    logger.info("Creating new instance specification.")
    instance_rddl_filepaths, graph_index_filepaths = create_instance(
        rddl_path=config["rddl_dirpath"],
        n_instances=1,
    ...
    logger.info(
        f'Animation data produced and written to {config["animation_sequence_dirpath"]}.'
    )
