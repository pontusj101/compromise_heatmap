...
class Evaluator:
    ...
    def print_results(
        ...
        test_predicted_labels = (test_predicted_probs > self.trigger_threshold).astype(
            int
        )
        ...
        logging.info(
            f"{methods} evaulation completed. Time: {time.time() - start_time:.2f}s."
        )
    ...
    def evaluate_test_set(
        ...
        predictor = Predictor(
            predictor_type, predictor_filename, bucket_manager=bucket_manager
        )
        sequence_filenames = get_sequence_filenames(
            bucket_manager,
            sequence_dir_path,
            min_nodes,
            max_nodes,
        ...
        logging.info(
            f"Evaluating {predictor_type} predictor {predictor_filename} on {len(sequence_filenames)} snapshot sequences."
        )
